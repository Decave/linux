...
static void test_user_ringbuf_mappings(void)
{
	...
	CHECK(!errno, "wr_prod_pos_err", "Was able to remap producer pointer to large size\n");
	ASSERT_OK(munmap(mmap_ptr, page_size), "unmap_ro_cons");

	/* prod_pos can be mapped RW, can't add +X with mprotect. */
	mmap_ptr = mmap(NULL, page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rb_fd, page_size);
	ASSERT_OK_PTR(mmap_ptr, "rw_prod_pos");
	ASSERT_ERR(mprotect(mmap_ptr, page_size, PROT_EXEC), "exec_prod_pos_protect");
	CHECK(!errno, "wr_prod_pos_err", "Was able to map producer pointer as executable\n");
	...
	CHECK(!errno, "exec_data_err", "Was able to map data pages as executable\n");
...
static void test_user_ringbuf_overfill(void)
{
	...
	CHECK(!errno, "write_samples_errno", "too_many_samples_posted");
